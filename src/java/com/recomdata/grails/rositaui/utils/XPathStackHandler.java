...
public class XPathStackHandler {
	...
	public String getXPath() {
		...
		XPathStackEntry[] entryArray = entries.toArray(new XPathStackEntry[0]); //Avoid ConcurrentModificationException
		for (int i = 1; i < entryArray.length; i++) { //Don't output the first entry - this is root.
			XPathStackEntry entry = entryArray[i];
			if (entry != null) {
