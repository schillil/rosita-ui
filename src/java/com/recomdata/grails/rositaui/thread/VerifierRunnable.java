...
public class VerifierRunnable implements Runnable {
	...
	public void run() {
		try {
			...
			String scriptfile;
			if(filename.lastIndexOf(".xml") == filename.length() - 4) scriptfile = "./" + "verifyfiles" + suffix; //Ensures that a Unix process can start, even if . is not in PATH
			else scriptfile = "verifycsv" + suffix;
			ProcessBuilder pb = new ProcessBuilder(scriptPath + scriptfile, filename, "forui");
			canonicalPath = new File(scriptPath).getCanonicalPath();
			pb.directory(new File(canonicalPath));
			
			Process verifyProcess = pb.start();
			BufferedReader r = new BufferedReader(new InputStreamReader(verifyProcess.getInputStream()));
			String line;
			...
			System.out.println("...Finished reading. Process exit code was " + verifyProcess.waitFor());
			sig.sendConsole(stepId, "END");
			exitCode = verifyProcess.waitFor();
			sig.sendSignal(jobId, 1, exitCode == 0);
		}
		catch (Exception e) {
