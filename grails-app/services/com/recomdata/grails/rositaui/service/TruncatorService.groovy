...
class TruncatorService {
    ...
    def start(RositaJob j, WorkflowStep wf) {
		if (!truncator.isAlive()) {
			...
			truncator.setName("Truncation Thread")
			truncator.start();
			return "Truncation started"
		}
		else {
			return "The truncator is already running!"
		}
    }
	...
	def cancelValidation() {
		if (truncator.isAlive()) {
			truncator.interrupt();
			return ("Truncation cancelled");
		}
		else {
			return "No truncator was running";
		}
	}
