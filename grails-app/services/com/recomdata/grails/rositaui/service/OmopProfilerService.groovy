...
import com.recomdata.grails.rositaui.thread.OmopProfilerRunnable
import org.codehaus.groovy.grails.commons.ConfigurationHolder;

class OmopProfilerService {

    static transactional = true
	
	static Thread profiler = new Thread()
	static Runnable myRunnable = new OmopProfilerRunnable()
    ...
    def start(RositaJob j, WorkflowStep wf) {
		if (!profiler.isAlive()) {
			...
			profiler.setName("OMOP Profiling Thread")
			profiler.start();
